...
from share_encoder_model import CorefModel
import conll
import sys
import os
...
class Runner:
    def __init__(self, config_name, gpu_id=0, seed=None):
        ...
        self.training_mode = 'intermediate'
    ...
    def initialize_model(self, saved_suffix=None):
        model = CorefModel(self.config, self.device, self.data.get_wn_embed(), self.data.get_ent_embed(), self.name)
        if saved_suffix:
    ...
    def evaluate(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None):
        ...
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            ...
            tensor_example = tensor_example[:input_tensor_num]  # Strip out gold
            example_gpu = [d.to(self.device) for d in tensor_example]
            with torch.no_grad(): # output predictions
                span_starts, span_ends, antecedent_idx, antecedent_scores = model(*example_gpu)
    ...
    def predict(self, model, tensor_examples):
        ...
        for i, tensor_example in enumerate(tensor_examples):
            tensor_example = tensor_example[:input_tensor_num]
            example_gpu = [d.to(self.device) for d in tensor_example]
            with torch.no_grad():
                span_starts, span_ends, antecedent_idx, antecedent_scores = model(*example_gpu)
            span_starts, span_ends = span_starts.tolist(), span_ends.tolist()
