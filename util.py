...
def parse_config(config_name):
    ...
    config = pyhocon.ConfigFactory.parse_file("bi_encoder_experiments.conf")[config_name]
    config['log_dir'] = join(config["log_root"], config_name)
    makedirs(config['log_dir'], exist_ok=True)
...
def load_voca_embs(voca_path, embs_path, insert_unk=True):
    voca = Vocabulary.load(voca_path, insert_unk)
    embs = np.load(embs_path)

    # check if sizes are matched
    if embs.shape[0] == voca.size() - 1:
        # unk token is inserted, thus insert the embedding of unk token
        unk_emb = np.mean(embs, axis=0, keepdims=True)
        embs = np.append(embs, unk_emb, axis=0), # embeddings of unknown words
