...
class CorefDataProcessor:
    def __init__(self, config, language='english'):
        ...
        if os.path.exists(cache_path):
            ...
            self.tensor_samples = {}
            tensorizer = Tensorizer(self.config, self.wn_vocab, self.ent_vocab)
            paths = {
    ...
    @classmethod
    def convert_to_torch_tensor(cls, input_ids, input_mask, speaker_ids, word_ids, ent_ids, subtoken_map, sentence_len, genre, sentence_map,
    ...
    def get_intermediate_tensors(self):
        cache_path = join(self.data_dir, f'cached.intermediate.tensors.{self.language}.{self.max_seg_len}.{self.max_training_seg}.bin')
        if os.path.exists(cache_path):
            # Load cached tensors if exists
            with open(cache_path, 'rb') as f:
                self.tensor_samples, self.stored_info = pickle.load(f)
                logger.info('Loaded cached intermediate tensorized examples from cache')
            return self.tensor_samples['trn'], self.tensor_samples['dev'], self.tensor_samples['tst']
        else:
            return None
    ...
    def get_wn_embed(self):
        return self.wn_embed

    def get_ent_embed(self):
        return self.ent_embed

class Tensorizer:
    def __init__(self, config, word2id, ent2id):
        self.config = config
    ...
    def get_entity_ids(self, word_list, ent2id=None):
        # only consider the capitalized Named Entity Mentions
        nem_start = []
        nem_end = []
        ent_ids = [-1] * len(word_list)
        if ent2id is None:
            return np.array(ent_ids)
        for i, word in enumerate(word_list):
            if word[0].isupper() and i == 0:
                nem_start.append(i)
            elif word[0].isupper() and len(nem_start) == len(nem_end):
                nem_start.append(i)
            elif (not word[0].isupper()) and len(nem_start) != len(nem_end):
                nem_end.append(i)
        if len(nem_start) != len(nem_end):
            nem_end.append(len(word_list))
        for start, end in zip(nem_start, nem_end):
            nem_len = end - start
            # named entity mentions longer than one word
            if nem_len != 1:
                nem = '_'.join(word_list[start:end])
                ent_id = ent2id.get_id(nem)
    ...
    def tensorize_example(self, example, is_training):
        ...
        # clusters = [[[42, 43], [198, 199], [262, 263], [506, 507]], [[33, 38], [29, 29]]]
        gold_mentions = sorted(tuple(mention) for mention in util.flatten(clusters))
        #gold_mentions = [(24, 25), (29, 29), (33, 38), (42, 43)],
        ...
        # word without word pieces, for WOrdNET
        wn_words = example['tokens']
        if self.vocab is not None:
            word_ids = np.array([self.vocab.get_id(word) for word in wn_words])
        else:
            word_ids = np.array([1] * len(wn_words))
        # seudo linking
        ent_ids = self.get_entity_ids(wn_words, self.entities)
        # subtoken map, map to the original words
        subtoken_map = np.array(example['subtoken_map'])
        ...
        example_tensor = (input_ids, input_mask, speaker_ids, word_ids, ent_ids, subtoken_map, sentence_len, genre, sentence_map, is_training,
