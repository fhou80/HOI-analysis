...
class CorefModel(nn.Module):
    def __init__(self, config, device, wn_embed, ent_embed, config_name, num_genres=None):
        ...
        # WOrdNet embeddings
        if wn_embed is not None and config['use_ent_info']:
        ...
        # Entity Embeddings
        if ent_embed is not None and config['use_ent_info']:
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, word_ids, ent_ids, subtoken_map, sentence_len, genre, sentence_map,
        ...
        # Get token emb
        if 'electra' in self.config_name:
            mention_doc, hidden_layers = self.bert(input_ids, attention_mask=input_mask, output_hidden_states=True)  # [num seg, num max tokens, emb size]
            mention_doc_one = hidden_layers[1]
        elif 'spanbert' in self.config_name:
            mention_doc, _, hidden_layers = self.bert(input_ids, attention_mask=input_mask, output_hidden_states=True)  # [num seg, num max tokens, emb size]
            mention_doc_one = hidden_layers[1]
        ...
        antecedent_offsets = torch.unsqueeze(top_span_range, 1) - torch.unsqueeze(top_span_range,
                                                                                  0)  # [num_top_spans, num_top_spans]
        ...
        if conf['fine_grained']:
            ...
            target_emb = torch.unsqueeze(top_span_emb, 1).repeat(1, max_top_antecedents,
                                                                 1)  # [num_top_spans, mant_top_antecedents, emb_dim]
